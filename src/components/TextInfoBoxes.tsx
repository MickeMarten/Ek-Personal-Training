...
function InfoBoxes({text, className}:TextInfoBoxesProps) {
  return (
    <div className={`px-4 py-4 ${className ? className : "xl:self-center"}`}>
  ...
  <Blockquote className={`text-lg xl:text-2xl font-semibold text-primary xl:min-w-96 ${className ? className : ""}`}>
    {text}
  </Blockquote>
</div>
